...
int cdfs_toc_read_cd( struct super_block *sb , int mounting_acd)
{
	...
	{
        this_cd->track[i].start_lba  = 0;
        this_cd->track[i].stop_lba   = 0;
	}
	*/
		...
		if(mounting_acd)
			this_cd->track[i].type = 1;
		else
			this_cd->track[i].type = 2;
...
int isACD(struct super_block *sb)
{
	int i, t;
	int result=0;
	struct cdrom_tochdr hdr;
	...
	*/
	...
	kfree(this_cd);	//T13
	PRINTT("isEACD_SP and result=%d\n",result);
	//if(result==this_cd->tracks)
	//if(result==2)
	if((result==2)||((this_cd->tracks==1)&&(result==1)))
	{
		PRINTT("This is a Music CD\n");
		return 1;
	}
	else
	{
		PRINTT("This is NOT a Music CD\n");
		return 0;
	}
}
