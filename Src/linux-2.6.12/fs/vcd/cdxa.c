...
int vcdfs_get_xa_sector_info(struct super_block * sb, unsigned  int start_lba, unsigned int *data_size, unsigned int *data_offset)
{
	...
	*data_offset=0;
	...
	frame = kmalloc(2348, GFP_KERNEL);	//C01
	PRINTC("cdxa.c:vcdfs_get_sector_info() CD_TYPE=%d start_lba=%u First_Video_LBA=%d\n",CD_TYPE,start_lba,First_Video_LBA);
	
	if((start_lba>=First_Video_LBA)&&((CD_TYPE==VCD_DISC)||(CD_TYPE==AVCD_DISC)||(CD_TYPE==SVCD_DISC)))
	{
		*data_size=2352;
		PRINTC("M2F2 data_size=%d(direct)\n",*data_size);
		*data_offset = 8;
	}
	/*
