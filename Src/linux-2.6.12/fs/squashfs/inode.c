...
static DECLARE_MUTEX(squashfs_dec_tmp_sem);
unsigned char squashfs_dec_tmp[2*BLOCK_SIZE];
				...
				int *cur_index, int *offset, int *c_byte)
{
	...
	down(&squashfs_dec_tmp_sem);
	if(platform_info.secure_boot) {
		MCP_AES_ECB_Decryption(platform_info.AES_IMG_KEY, bh->b_data, squashfs_dec_tmp, bh->b_size);
		buffer_ptr = squashfs_dec_tmp;
	} else
			...
			long long *next_index)
{
	...
	for (bytes = 0, k = 0; k < b; k++) {
		...
		down(&squashfs_dec_tmp_sem);
		if(platform_info.secure_boot) {
			MCP_AES_ECB_Decryption(platform_info.AES_IMG_KEY, bh[k]->b_data, squashfs_dec_tmp, bh[k]->b_size);
			buffer_ptr = squashfs_dec_tmp;
		} else
