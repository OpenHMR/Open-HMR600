		...
		unsigned long len, unsigned long pgoff, unsigned long flags)
{
	...
	if (!vmm || addr + len <= vmm->vm_start) {
		...
		printk("%s(%d): find a conflicting vma: %x \n", __FUNCTION__, __LINE__, vmm->vm_start);
		BUG();
		return -EINVAL;
	}
/*
...
unsigned long pli_map_memory(unsigned long phy_addr, int size)
{
	...
	caddr = do_mmap(NULL, phy_addr+DEF_MAP_ADDR, size, PROT_READ | PROT_WRITE, 
		MAP_PRIVATE | MAP_NORESERVE | MAP_ANONYMOUS, 0);
	BUG_ON(caddr & 0x80000000);
	gaddr = do_mmap(NULL, phy_addr+DEF_MAP_ADDR+DEF_MEM_SIZE*1, size, PROT_READ | PROT_WRITE, 
		MAP_PRIVATE | MAP_NORESERVE | MAP_ANONYMOUS, 0);
	BUG_ON(gaddr & 0x80000000);
	uaddr = do_mmap(NULL, phy_addr+DEF_MAP_ADDR+DEF_MEM_SIZE*2, size, PROT_READ | PROT_WRITE, 
		MAP_PRIVATE | MAP_NORESERVE | MAP_ANONYMOUS, 0);
	BUG_ON(uaddr & 0x80000000);

	printk("+++ map phy: %x vir: %x \n", phy_addr, caddr);
