...
#ifndef CONFIG_REALTEK_ADVANCED_RECLAIM
struct page *dvr_reserved_start = 0;
unsigned int dvr_reserved_count = 0;
#endif
			 ...
			 struct address_space *mapping)
{
	...
	newpage->flags |= page->flags & ~(1 << PG_uptodate) & ~(1 << PG_writeback) &
	    ~(1 << PG_highmem) & ~(1 << PG_active) & ~(1 << PG_nfsdirty) & ~(~0UL << NODEZONE_SHIFT);

	BUG_ON(PageNFSDirty(newpage));
...
#else
	if ((newpage = ops->remap_alloc_page(GFP_DVRUSER | __GFP_HUGEFREE)) == NULL)
#endif
		return -ENOMEM;
...
#else
int remap_DVR_region(struct page *page, unsigned int count)
{
	struct zone *zone = zone_table[ZONE_DVR];
	int i, nr_done = 0, nr_fail = 0, retvalue;
	LIST_HEAD(failedp);

	printk("1. start remap DVR zone %p(%p) %d...\n", page_address(page), page, count);
	lru_add_drain();				// flush pagevec
	invalidate_bh_lrus();
	drain_pcp_pages(smp_processor_id());		// flush per_cpu_pages
	...
	for (i = 0; i < count; i++, page++) {
		if (page_count(page) != 0) {
			...
			if (!TestClearPageLRU(page)) {
				/* the page is in isolate_lru_pages(); */
				__put_page(page);
				spin_unlock_irq(&zone->lru_lock);
				nr_fail++;
				continue;
			...
			if ((retvalue = remap_onepage(page, 1, &remap_ops))) {
				nr_fail++;
				list_add(&page->lru, &failedp);
				if (retvalue == -ENOMEM) {
					printk("remap: not enough memory...\n");
...
struct page *start_remap(unsigned int order)
{
#ifndef CONFIG_REALTEK_MARS_256MB
	struct zonelist zonelist = {{0}};
#endif
	...
	down_interruptible(&remap_sem);

	// here, we are in charge of doing the remapping...
	if (order == 0) {
		// remap all DVR region...
		zone->flags |= ZN_DISABLE;
#ifndef CONFIG_REALTEK_MARS_256MB
		zonelist.zones[0] = zone;
		try_to_free_pages(zonelist.zones, __GFP_IO | __GFP_FS | __GFP_HUGEFREE, 0, 0);
		...
		// remap only the region we need...
#ifndef CONFIG_REALTEK_MARS_256MB
		zonelist.zones[0] = zone;
		try_to_free_pages(zonelist.zones, __GFP_IO | __GFP_FS | __GFP_HUGEFREE, 0, 0);
#endif
		page = alloc_dvrbp_memory(order);
		if (page == NULL)
			return NULL;
		dvr_reserved_start = page;
		dvr_reserved_count = 1 << order;
		while (remap_DVR_region(page, 1 << order)) {
			printk("\tretry remapping...\n");
			msleep(10);
		}
		if (order > page->private) {
			printk(" ### REQ: %d GOT: %d \n", order, page->private);
			show_dvrbp();
			BUG();
		}
...
void end_remap(unsigned int order)
{
	if (order == 0) {
		struct zone *zone = zone_table[ZONE_DVR];
		zone->flags &= ~ZN_DISABLE;
	} else if (order < MAX_ORDER) {
		dvr_reserved_start = 0;
		dvr_reserved_count = 0;
	}

	// here, we release the control of remapping...
	up(&remap_sem);
}
