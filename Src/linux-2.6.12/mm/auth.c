...
//						printk("free address: %x, order: %d \n", addr, order-i);
				...
				free_dvrbp_memory(virt_to_page((void *)pRecords[cnt][1]), pRecords[cnt][0] & 0x000000ff);
				up(&remap_sem);
#endif
			}
			start_index = pRecords[cnt][1];
		...
		case AUTH_IOCQALLOC:
			...
			if (arg <= max_cache_size) {
				...
				if (order == MAX_ORDER-1) {
					// special case for 16MB allocation...
					show_buddy_info();
					ret = __get_free_pages(GFP_USER | __GFP_NOWARN | __GFP_HUGEFREE, order);
					show_buddy_info();
				}
				if (!ret) {
#else
...
#endif
					struct page *page;
					page = start_remap(order);
					if (page != 0) {
						ret = __get_free_pages(GFP_DVRUSER | __GFP_NOWARN | 
								__GFP_EXHAUST | __GFP_HUGEFREE, order);
						BUG_ON((int)page_address(page) != ret);
					} else
						ret = 0;
					end_remap(order);
...
//						printk("cnt value: %d \n", cnt);
					...
					if (record_insert(pli_signature | buddy_id | order, (unsigned long)ret)) {
						down_interruptible(&remap_sem);
						free_pages((unsigned long)ret, order);
						free_dvrbp_memory(virt_to_page((void *)ret), order);
						up(&remap_sem);
						return 0;
					}
#endif
...
//							printk("free address: %x, order: %d \n", arg, order-i);
					...
					down_interruptible(&remap_sem);
					free_pages((unsigned long)arg, order);
					free_dvrbp_memory(virt_to_page((void *)arg), order);
					up(&remap_sem);
#endif
				}
			}
