...
static void init_mlme_ext_priv_value(_adapter* padapter)
{
	...
	pmlmeext->cur_channel = padapter->halpriv.CurrentChannel;
	pmlmeext->cur_bwmode = HT_CHANNEL_WIDTH_20;
	pmlmeext->cur_ch_offset = HAL_PRIME_CHNL_OFFSET_DONT_CARE;

	_memset(pmlmeext->channel_set, 0, NUM_CHANNELS);
	printk("%s(): channel plane = %x \n",__FUNCTION__,padapter->mlmepriv.ChannelPlan);
...
unsigned int OnBeacon(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	{
		process_phy_info(padapter, precv_frame);
...
unsigned int OnAuthClient(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	algthm 	= (*(unsigned short *)((unsigned int)pframe + WLAN_HDR_A3_LEN + offset));
	seq 	= (*(unsigned short *)((unsigned int)pframe + WLAN_HDR_A3_LEN + offset + 2));
	status 	= (*(unsigned short *)((unsigned int)pframe + WLAN_HDR_A3_LEN + offset + 4));
...
unsigned int OnAssocRsp(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	//status
	if ((status = (*(unsigned short *)(pframe + WLAN_HDR_A3_LEN + 2))) > 0)
	{
	...
	//get capabilities
	caps = (*(unsigned short *)(pframe + WLAN_HDR_A3_LEN));

	//set slot time
	pmlmeinfo->slotTime = (caps & BIT(10))? 9: 20;

	//AID
	res = (int)((*(unsigned short *)(pframe + WLAN_HDR_A3_LEN + 4)) & 0x3fff);
...
void issue_auth(_adapter *padapter, struct sta_info *psta, unsigned short status)
{
	...
	*(fctrl) = 0;
	...
	{		
		...
		val16 = (pmlmeinfo->auth_algo == 1)? 1: 0;
		if (val16)	{
...
void site_survey(_adapter *padapter)
{
	...
	pmlmeext->survey_channel = pmlmeext->channel_set[pmlmeext->sitesurvey_res.channel_idx];

	if (pmlmeext->survey_channel != 0)
	{
		//SelectChannel(padapter, survey_channel, HAL_PRIME_CHNL_OFFSET_DONT_CARE);
		set_channel_bwmode(padapter, pmlmeext->survey_channel, HAL_PRIME_CHNL_OFFSET_DONT_CARE, HT_CHANNEL_WIDTH_20);
		...
		_set_timer(&pmlmeext->survey_timer, SURVEY_TO);
...
u8 collect_bss_info(_adapter *padapter, union recv_frame *precv_frame, WLAN_BSSID_EX *bssid)
{
	...
	bssid->Rssi = precv_frame->u.hdr.attrib.signal_strength;
	...
	{// some ap do not have DSConfig IE, driver use current survey channel.
		if(pmlmeext->survey_channel !=0 )
			bssid->Configuration.DSConfig = pmlmeext->survey_channel;
		else
			bssid->Configuration.DSConfig = pmlmeext->cur_channel;
	}
	//DBG_871X("%s() ssid = %s, channel = %d\n", __FUNCTION__,bssid->Ssid.Ssid, bssid->Configuration.DSConfig);
	_memcpy(&bssid->Configuration.BeaconPeriod, get_beacon_interval_from_ie(bssid->IEs), 2);

	val16 = get_capability((NDIS_WLAN_BSSID_EX *)bssid);
...
void start_clnt_join(_adapter* padapter)
{
	...
	caps = get_capability((NDIS_WLAN_BSSID_EX *)pnetwork);
	if (caps&cap_ESS)
	{
		...
		(pmlmeinfo->auth_algo == 2)? write8(padapter, REG_SECCFG, 0xcc): write8(padapter, REG_SECCFG, 0xcf);
...
void linked_status_chk(_adapter *padapter)
{
	...
	if (padapter->mlmepriv.fw_state & _FW_UNDER_SURVEY)
		return;
	...
	if (is_client_associated_to_ap(padapter))
	{
		...
		if ((psta = get_stainfo(pstapriv, pmlmeinfo->network.MacAddress)) != NULL)
		{
			...
			{
				DBG_871X("(%x)linked_status_chk():receive_disconnect\n",padapter->halpriv.interfaceIndex);
			...
			else if ((pmlmeinfo->link_count & 0xf) == 0)
			{
				...
				{
					DBG_871X("(%x)issue nulldata to keep alive\n",padapter->halpriv.interfaceIndex);
					issue_nulldata(padapter, 0);
				}
