...
void dm_DynamicTxPower(IN	PADAPTER	Adapter)
{
	...
	if( (pdmpriv->bDynamicTxPowerEnable != _TRUE) ||
		(!(pdmpriv->DMFlag & HAL_DM_HIPWR_DISABLE)) )		
	{
		pdmpriv->DynamicTxHighPowerLvl = TxHighPwrLevel_Normal;
		return;
	}
	...
	if(IS_HARDWARE_TYPE_8192D(pHalData) && pHalData->CurrentBandType92D == 1){
...
{
#if 0
	PADAPTER		ExtAdapter = Adapter;//ADJUST_TO_ADAPTIVE_ADAPTER(Adapter,_FALSE);
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
	struct dm_priv	*pdmpriv = &Adapter->dmpriv;
...
//	static u32			EDCA_BE_reg=0;
	static u8			LastHighPwrLvl = 0xff;
	static u8			LastBtOn = 0xff;
	static u8			LastBtSco = 0xff;
	//[Win7 Count Tx/Rx statistic for Extension Port] dm_CheckEdcaTurbo's Adapter is always Default. 2009.08.20, by Bohn
	
	static u64			Ext_lastTxOkCnt = 0;
	static u64			Ext_lastRxOkCnt = 0;
	u64				Ext_curTxOkCnt = 0;
	u64				Ext_curRxOkCnt = 0;

	//For future Win7  Enable Default Port to modify AMPDU size dynamically, 2009.08.20, Bohn.	
	//u8 TwoPortStatus = (u8)TWO_PORT_STATUS__WITHOUT_ANY_ASSOCIATE;
	...
	//RT_TRACE(COMP_TURBO,DBG_TRACE,("==>dm_CheckEdcaTurbo, Now is %s Adapt.\n",(Adapter==ADJUST_TO_ADAPTIVE_ADAPTER(Adapter,TRUE))?"Def":"Ext"));
	//RT_TRACE(COMP_TURBO,DBG_TRACE,("Def_IOTPeer=%ld.Ext_IOTPeer=%ld\n",DefAdapter->MgntInfo.IOTPeer, ExtAdapter->MgntInfo.IOTPeer));	
		...
		(pHalData->bt_coexist.BT_Ant_isolation))
	{
		...
		if(pHalData->DynamicTxHighPowerLvl == TxHighPwrLevel_Normal)
		{
			...
			{
				RTPRINT(FBT, BT_TRACE, ("BT in BT_OtherBusy state Tx %ld  \n",pHalData->bt_coexist.Ratio_Tx));
...
*/			
			...
			else if(pHalData->bt_coexist.BT_Service==BT_SCO)
			{
				if(pHalData->bt_coexist.Ratio_Tx>160)
				{
					EDCA_BE_UL = 0x5ea72f;
					EDCA_BE_DL = 0x5ea72f;
					RTPRINT(FBT, BT_TRACE, ("BT in BT_SCO state Tx (%ld) >160 parameter(0x%x) = 0x%x\n",pHalData->bt_coexist.Ratio_Tx, REG_EDCA_BE_PARAM, 0x5ea72f));
				}
				else
				{
					EDCA_BE_UL = 0x5ea32b;
			...
			else if((pHalData->bt_coexist.BT_Service==BT_Idle) ||(pHalData->bt_coexist.BT_Service==BT_OtherAction))
			{
					...
					//EDCA_BE_UL = 0x5ea32b;
					//EDCA_BE_DL = 0x5ea42b;
					RTPRINT(FBT, BT_TRACE, ("BT in BT_Idle  or BT_OtherAction state parameter(0x%x) = 0x%lx\n", REG_EDCA_BE_PARAM, EDCA_BE_UL));		
			}
			else
			{
...
*/
	...
	if((pMgntInfo->IOTAction & HT_IOT_ACT_DISABLE_EDCA_TURBO) ||pdmpriv->bForcedDisableTurboEDCA)
		goto dm_CheckEdcaTurbo_EXIT;

#if (DEV_BUS_TYPE == PCI_INTERFACE)
	if((!pdmpriv->bDisableFrameBursting) && (!pHalData->bt_coexist.BluetoothCoexist) &&
		(pMgntInfo->IOTAction & (HT_IOT_ACT_FORCED_ENABLE_BE_TXOP|HT_IOT_ACT_AMSDU_ENABLE)))
#else
	if((!pdmpriv->bDisableFrameBursting) &&
		(pMgntInfo->IOTAction & (HT_IOT_ACT_FORCED_ENABLE_BE_TXOP|HT_IOT_ACT_AMSDU_ENABLE)))
#endif
	...
	if((!pHalData->bIsAnyNonBEPkts) && (!pMgntInfo->bDisableFrameBursting))
	{
		...
		Ext_curTxOkCnt = ExtAdapter->TxStats.NumTxBytesUnicast - Ext_lastTxOkCnt;
		Ext_curRxOkCnt = ExtAdapter->RxStats.NumRxBytesUnicast - Ext_lastRxOkCnt;
...
dm_CheckEdcaTurbo_EXIT:
	...
	Ext_lastTxOkCnt = ExtAdapter->TxStats.NumTxBytesUnicast;
	Ext_lastRxOkCnt = ExtAdapter->RxStats.NumRxBytesUnicast;
...
//		if(!pHalData->ThermalValue)
		...
		//RT_TRACE(COMP_POWER_TRACKING, DBG_LOUD,("Readback Thermal Meter = 0x%lx pre thermal meter 0x%lx EEPROMthermalmeter 0x%lx delta 0x%lx delta_LCK 0x%lx delta_IQK 0x%lx\n", ThermalValue, pHalData->ThermalValue, pHalData->EEPROMThermalMeter, delta, delta_LCK, delta_IQK));
		...
		if(pdmpriv->TxPowerTrackControl && delta != 0)
		{
			...
			//RTPRINT(FINIT, INIT_IQK, ("TxPwrTracking 0xc80 = 0x%x, 0xc94 = 0x%x RF 0x24 = 0x%x\n", PHY_QueryBBReg(Adapter, 0xc80, bMaskDWord), PHY_QueryBBReg(Adapter, 0xc94, bMaskDWord), PHY_QueryRFReg(Adapter, RF90_PATH_A, 0x24, bMaskDWord)));
		}
