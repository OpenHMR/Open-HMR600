...
u8 init_default_value(_adapter *padapter)
{
	...
	psecuritypriv->dot11AuthAlgrthm = 0; //open system
...
*/
	...
	_set_timer(&padapter->mlmepriv.dhcp_timer, 10000);
	while(1)
	{
		_cancel_timer(&padapter->mlmepriv.dhcp_timer, &bool);
		if (bool == _TRUE)
			break;
	}
	RT_TRACE(_module_os_intfs_c_,_drv_err_,("\n drv_halt:cancel dhcp_timer! \n"));
#endif
