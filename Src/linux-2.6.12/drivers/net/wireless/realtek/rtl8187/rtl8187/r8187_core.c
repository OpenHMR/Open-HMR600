...
int rtl8180_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	switch (cmd) {
		...
		if(  ipw->cmd == IPW_CMD_SET_ENCRYPTION ){
			if( ipw->u.crypt.set_tx && ((strcmp(ipw->u.crypt.alg, "TKIP")==0)  || (strcmp(ipw->u.crypt.alg, "CCMP")==0) ) )
			{
			...
			else if( ipw->u.crypt.set_tx && (strcmp(ipw->u.crypt.alg, "WEP")==0)){
				//enable HW security
				...
				key[2] = ((u32*)wrq->u.data.pointer)[14];
				key[3] = ((u32*)wrq->u.data.pointer)[15];
				printk("ipw Set SW WEP key, key index = %d\n",ipw->u.crypt.idx);
				if(ipw->u.crypt.key_len == 5)
				{
					ieee->broadcast_key_type = KEY_TYPE_WEP40;
					ieee->pairwise_key_type = KEY_TYPE_WEP40;
					setKey( dev,
						ipw->u.crypt.idx,//EntryNo
						ipw->u.crypt.idx,//KeyIndex 
						KEY_TYPE_WEP40,  //KeyType
						zero_addr[ipw->u.crypt.idx],
						0,              //DefaultKey
						key);           //KeyContent

					if(ipw->u.crypt.idx == 0)
					{
						setKey( dev,
							4,               //EntryNo
							ipw->u.crypt.idx,//KeyIndex 
