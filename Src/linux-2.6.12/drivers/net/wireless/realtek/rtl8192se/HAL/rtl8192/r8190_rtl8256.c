		...
		switch( eRFPowerState )
		{
			...
			case eRfOff:					 
				...
				rtl8192_setBBreg(dev, rFPGA0_XA_RFInterfaceOE, BIT4, 0x0);		// 0x860[4]
					...
					//2 AFE
				//analog to digital off, for power save
					//PHY_SetBBReg(dev, rFPGA0_AnalogParameter4, 0xf00, 0x0);// 0x88c[11:8]
					rtl8192_setBBreg(dev, rFPGA0_AnalogParameter4, 0xf03, 0x0); //  2008.09.30 Modify
				//digital to analog off, for power save
					//PHY_SetBBReg(dev, rFPGA0_AnalogParameter1, 0x18, 0x0); // 0x880[4:3]
					rtl8192_setBBreg(dev, rFPGA0_AnalogParameter1, 0x98, 0x0); // 0x880 2008.09.30 Modify
					//rx antenna off  2008.09.30 mark 
					//PHY_SetBBReg(dev, rOFDM0_TRxPathEnable, 0xf, 0x0);// 0xc04[3:0]
					//rx antenna off  2008.09.30 mark 
					//PHY_SetBBReg(dev, rOFDM1_TRxPathEnable, 0xf, 0x0);// 0xd04[3:0]
				//analog to digital part2 off, for power save
					...
					//PHY_SetBBReg(dev, rFPGA0_AnalogParameter1, 0x4, 0x0); // 0x880[2]
					//2 BB
					// Baseband reset 2008.09.30 add
	...
	)
{
	...
	u16					RFWaitCounter = 0;
	unsigned long flag;
	RT_TRACE(COMP_PS, "===>MgntActSet_RF_State(): StateToSet(%d)\n",StateToSet);
	...
	while(true)
	{
		...
		if(priv->RFChangeInProgress)
		{
			...
			RT_TRACE(COMP_PS, "MgntActSet_RF_State(): RF Change in progress! Wait to set..StateToSet(%d).\n", StateToSet);
			
			// Set RF after the previous action is done. 
			while(priv->RFChangeInProgress)
			{
				RFWaitCounter ++;
				RT_TRACE(COMP_PS, "MgntActSet_RF_State(): Wait 1 ms (%d times)...\n", RFWaitCounter);
				udelay(1000); // 1 ms
	...
	case eRfOn:
		...
		if(! priv->ieee80211->RfOffReason)
		{
			...
			RT_TRACE(COMP_PS, "MgntActSet_RF_State - eRfon reject pMgntInfo->RfOffReason= 0x%x, ChangeSource=0x%X\n", priv->ieee80211->RfOffReason, ChangeSource);
	...
	if(bActionAllowed)
	{
		RT_TRACE(COMP_PS, "MgntActSet_RF_State(): Action is allowed.... StateToSet(%d), RfOffReason(%#X)\n", StateToSet, priv->ieee80211->RfOffReason);
	...
	{
		RT_TRACE(COMP_PS, "MgntActSet_RF_State(): Action is rejected.... StateToSet(%d), ChangeSource(%#X), RfOffReason(%#X)\n", StateToSet, ChangeSource, priv->ieee80211->RfOffReason);
	}
	...
	RT_TRACE(COMP_PS, "<===MgntActSet_RF_State()\n");
