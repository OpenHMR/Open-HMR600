	...
	)
{
	...
	CmdInitTCB(dev, pTcb, pBuf, BufferLen);

	
	if(CmdCheckFragment(dev, pTcb, pBuf))
		CmdFragmentTCB(dev, pTcb);
	else
		pTcb->FragLength[0] = (u2Byte)pTcb->BufferList[0].Length;

	QueueID=pTcb->SpecifiedQueueID;
#if DEV_BUS_TYPE!=USB_INTERFACE	
	firstDesc=curDesc=dev->NextTxDescToFill[QueueID];
#endif
	
#if DEV_BUS_TYPE!=USB_INTERFACE
	if(VacancyTxDescNum(dev, QueueID) > pTcb->BufferCount)
#else
	if(PlatformIsTxQueueAvailable(dev, QueueID, pTcb->BufferCount) &&
		RTIsListEmpty(&dev->TcbWaitQueue[QueueID]))
#endif
	{
		pTcb->nDescUsed=0;
		...
		{
			dev->HalFunc.TxFillCmdDescHandler(
				dev,	
				pTcb,
				QueueID,							//QueueIndex 
				curDesc,							//index
...
#if DEV_BUS_TYPE!=USB_INTERFACE	
			curDesc=(curDesc+1)%dev->NumTxDesc[QueueID];
#endif
			pTcb->nDescUsed++;
		}

#if DEV_BUS_TYPE!=USB_INTERFACE
		RTInsertTailList(&dev->TcbBusyQueue[QueueID], &pTcb->List);
		IncrementTxDescToFill(dev, QueueID, pTcb->nDescUsed);
		dev->HalFunc.SetTxDescOWNHandler(dev, QueueID, firstDesc);
		// TODO: should call poll use QueueID
		dev->HalFunc.TxPollingHandler(dev, TXCMD_QUEUE);
#endif		
	}
	else
#if DEV_BUS_TYPE!=USB_INTERFACE	
...
#else
	{
		...
		RTInsertTailList(&dev->TcbWaitQueue[pTcb->SpecifiedQueueID], &pTcb->List);
	}
#endif
...
)
{
	...
	//Get TCB and local buffer from common pool. (It is shared by CmdQ, MgntQ, and USB coalesce DataQ)
	if(MgntGetBuffer(dev, &pTcb, &pBuf))
	{
		PlatformZeroMemory(pBuf->Buffer.VirtualAddress, Length);
		rtStatus = CmdSendPacket(dev, pTcb, pBuf, Length, DESC_PACKET_TYPE_INIT, bLastInitPacket);	//0 : always set LastInitPacket to zero
//#if HAL_CODE_BASE != RTL8190HW
//		// TODO: for test only
//		ReturnTCB(dev, pTcb, RT_STATUS_SUCCESS);
//#endif
...
#endif //#if (defined(RTL8192E) || defined(RTL8190P))
