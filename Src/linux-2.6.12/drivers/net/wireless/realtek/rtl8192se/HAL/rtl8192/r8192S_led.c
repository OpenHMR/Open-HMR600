...
Abstract:
	LED related implementation.
	    
Major Change History:
	When       Who               What
	---------- ---------------   -------------------------------
	2009-03-13 amy            Create.
...
void InitLed8190Pci(struct net_device *dev, PLED_8190 	pLed,LED_PIN_8190 LedPin)
{
	...
	pLed->BlinkTimer.data = (unsigned long)pLed;
...
//
//	Description:
//		DeInitialize an LED_8190 object.
//
void DeInitLed8190Pci(PLED_8190 pLed)
{
	//struct net_device *dev = (struct net_device *)pLed->dev;
	del_timer_sync(&(pLed->BlinkTimer));
	// We should reset bLedBlinkInProgress if we cancel the LedControlTimer.
	pLed->bLedBlinkInProgress = false;
}
...
//
//	Description:
//		Initialize all LED_8190 objects.
//
...
//
//	Description:
//		DeInitialize all LED_8190 objects.
//
...
void SwLedBlink(PLED_8190 pLed)
{
	...
	case LED_SCAN_BLINK:
		if( (priv->ieee80211->state == IEEE80211_LINKED) &&  // Linked.
			(!priv->ieee80211->be_scan_inprogress) && // Not in scan stage.
...
void SwLedControlMode7(struct net_device *dev, LED_CTL_MODE LedAction)
{
	...
	switch(LedAction)
	{
		...
		case LED_CTL_LINK:
		case LED_CTL_NO_LINK:			
			mod_timer(&(pLed0->BlinkTimer), jiffies + MSECS(0));			
			break;
...
//	return;
	...
	// Walk around for DTM test, we will not Check LED ON/OFF because r/w
...
//
//	Description:
//		Reschedule BlinkTimer when timer was not set successful before which resulted from 
//		system out of resource(when workitem and timer interworking), e.g, in Win98Me. 
//
//	Assumption:	
//		This function is only periodical invoked at HalDmWatchDog().
//
//	Created by Roger, 20070514.
//
...
//
//	Description:
//		Reschedule specific BlinkTimer when timer was not set successful before which resulted from 
//		system out of resource(when workitem and timer interworking), e.g, in Win98Me. 
//
//	Created by Roger, 20070516.
//
