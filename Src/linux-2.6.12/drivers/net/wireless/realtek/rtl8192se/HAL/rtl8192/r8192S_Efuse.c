...
ReadEFuse(struct net_device* dev, u16	 _offset, u16 _size_byte, u8 *pbuf)
{
	...
	u8  	offset, wren;
	u16  	i, j;
	u16 	eFuseWord[EFUSE_MAX_SECTION][EFUSE_MAX_WORD_UNIT];// = {0xFF};//FIXLZM
	//u16 	kindex = 0;
	...
	// 0. Refresh efuse init map as all oxFFFF.
...
#if (EFUSE_REPG_WORKAROUND == 1)
		// Only for 92SE check now.
		if (IS_HARDWARE_TYPE_8192SE(dev))
		{
			//
			// Efuse rePG workaround methid when we read EFUSE.
			//
			if (offset == 1)
			{
				sec1_addr = eFuse_Addr;	// Rcord section 1 PG header address.
			}
			else if (offset == 5)
			{
			...
			//
			// If section 3 is written before section 1, we do not forece write sectin 1 again.
			// Other
			//
...
//
// 2008/12/22 MH Add to rePG section data.
//
extern	void 
EFUSE_RePgSection1(struct net_device* dev)
{
	...
	//
	// 6. We need to rePG section 1( WORD 1) after all content.
	//
	if (priv->efuse_RePGSec1Flag == TRUE)
	{
		...
		//<Roger_Notes> We only re-program WORD 1 in section 1 to reduce PG size.
