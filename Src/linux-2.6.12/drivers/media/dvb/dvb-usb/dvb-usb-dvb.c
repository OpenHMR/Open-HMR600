...
int dvb_usb_dvb_init(struct dvb_usb_device *d)
{
    ...
    dvb_usb_streamer_init(&d->streamer, d);	    
#endif	
...
int dvb_usb_passthrough_control(struct dvb_frontend* fe, dvb_passthrough_cmd cmd, dvb_passthrough_mode mode)
{
    struct dvb_usb_device *d = fe->dvb->priv;    
    return dvb_usb_streamer_control(&d->streamer, cmd, mode);
}

int dvb_usb_set_passthrough_buffer(struct dvb_frontend* fe, unsigned char* pbuff, unsigned long size)
{
    struct dvb_usb_device *d = fe->dvb->priv;    
    return dvb_usb_streamer_set_buffer(&d->streamer, pbuff, size);
}

int dvb_usb_read_passthrough_data(struct dvb_frontend* fe, unsigned char** ppbuff, unsigned long* psize)
{
    struct dvb_usb_device *d = fe->dvb->priv;    
    return dvb_usb_streamer_read_data(&d->streamer, ppbuff, psize);
}

int dvb_usb_release_passthrough_data(struct dvb_frontend* fe, unsigned char* pbuff, unsigned long size)
{
    struct dvb_usb_device *d = fe->dvb->priv;    
    return dvb_usb_streamer_release_data(&d->streamer, pbuff, size);
}



int dvb_usb_get_pid_count(struct dvb_frontend *fe)
{
    struct dvb_usb_device *d = fe->dvb->priv;        

	if (d->props.caps & DVB_USB_HAS_PID_FILTER)
	    return d->props.pid_filter_count;

    return 0;
}


int dvb_usb_pid_filter_ctrl(struct dvb_frontend* fe, int on_off)
{
    struct dvb_usb_device *d = fe->dvb->priv;        

	if (d->props.pid_filter_ctrl)
	    return d->props.pid_filter_ctrl(d, on_off);

    return -1;	    
}


int dvb_usb_pid_filter(struct dvb_frontend *fe, int id, u16 pid, int on_off)
{
    struct dvb_usb_device *d = fe->dvb->priv;        

	if (d->props.pid_filter)
	    return d->props.pid_filter(d, id, pid, on_off);
...
int dvb_usb_fe_init(struct dvb_usb_device* d)
{
	...
	if (d->fe != NULL) {
	    ...
	    d->fe->ops->passthrough_control      = dvb_usb_passthrough_control;
	    d->fe->ops->set_passthrough_buffer   = dvb_usb_set_passthrough_buffer;
	    d->fe->ops->read_passthrough_data    = dvb_usb_read_passthrough_data;
	    d->fe->ops->release_passthrough_data = dvb_usb_release_passthrough_data;
	    
        if (d->fe->ops->get_pid_count==NULL)
	        d->fe->ops->get_pid_count = dvb_usb_get_pid_count;
	        
        if (d->fe->ops->pid_filter_ctrl==NULL)
	        d->fe->ops->pid_filter_ctrl = dvb_usb_pid_filter_ctrl;
	    
        if (d->fe->ops->pid_filter==NULL)
	        d->fe->ops->pid_filter = dvb_usb_pid_filter;
