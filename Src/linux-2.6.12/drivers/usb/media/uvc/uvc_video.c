	...
	struct usb_host_endpoint *ep)
{
	...
	for (i = 0; i < UVC_URBS; ++i) {
		...
		if (urb == NULL) {
		    printk("<UVC> uvc_video_init_iso failed - allocate urb failed...\n");
		...
		if (video->urb_buffer[i] == NULL) {
		    printk("<UVC> uvc_video_init_iso failed - allocate urb buffer failed...\n");
			usb_free_urb(urb);
			uvc_uninit_video(video);
			return -ENOMEM;
		}
...
int uvc_video_enable(struct uvc_video_device *video, int enable)
{
	...
	if ((ret = uvc_queue_enable(&video->queue, 1)) < 0){
	    printk("<UVC> uvc_video_enable failed - enable queue failed...\n");
		return ret;
	}
    
    if ((ret=uvc_init_video(video))<0)
        printk("<UVC> uvc_video_enable failed - uvc_init_video failed...\n");
