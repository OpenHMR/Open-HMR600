		     ...
		     unsigned int cmd, void *arg)
{
	...
	case VIDIOC_STREAMON:
	{
		...
		if (!uvc_has_privileges(handle)) {
		    printk("<UVC> StreamOn failed - UVC Does not have privileges...\n");
			return -EBUSY;
        }			

		if ((ret = uvc_video_enable(video, 1)) < 0)
		    printk("<UVC> StreamOn failed - video Enable failed...\n");
			return ret;
		break;
	}
...
static int uvc_v4l2_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	struct page *page;
	unsigned long addr, start, size, pfn;
	unsigned int i;
	int ret = 0;
