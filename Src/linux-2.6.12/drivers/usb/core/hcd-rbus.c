			  ...
			  struct platform_device *pdev)
{
	...
	if(strstr(buf, "ehci_hcd")){	//EHCI
		...
		// if ehci interrupt is set
		if(inl(VENUS_USB_EHCI_USBINTR)) {
			printk("#######[cfyeh-debug] %s(%d) inl(VENUS_USB_EHCI_USBINTR) 0x%x\n", __func__, __LINE__, inl(VENUS_USB_EHCI_USBINTR));
			outl(0, VENUS_USB_EHCI_USBINTR);
			printk("#######[cfyeh-debug] %s(%d) inl(VENUS_USB_EHCI_USBINTR) 0x%x\n", __func__, __LINE__, inl(VENUS_USB_EHCI_USBINTR));
		}

		// if ehci host is running
		if(inl(VENUS_USB_EHCI_USBCMD) & 0x1) {
			printk("#######[cfyeh-debug] %s(%d) inl(VENUS_USB_EHCI_USBCMD) 0x%x\n", __func__, __LINE__, inl(VENUS_USB_EHCI_USBCMD));
			outl(inl(VENUS_USB_EHCI_USBCMD) & ~(1<<0), VENUS_USB_EHCI_USBCMD);
			printk("#######[cfyeh-debug] %s(%d) inl(VENUS_USB_EHCI_USBCMD) 0x%x\n", __func__, __LINE__, inl(VENUS_USB_EHCI_USBCMD));
		}
	}
	else	//OHCI
