    ...
    )
{
    ...
    {
        printk("[MD] WARNING, no more entry\n");
        req = NULL;
    }        
...
{               
    ...
    if (handle >= dev->finished_copy_counter)   // task does not finished yet...
    {
        ...
        while(md_checkfinish(request_id)==0)                 
        {
            if (aaa++ > 100000)        
            {
                printk("[MD] WARNING, command not complete, req_id = %llu, hw_counter=%lu", request_id, readl(IOA_SMQ_INST_CNT) & 0xFFFF);
                aaa = 0;
            }                
            udelay(1);        
        }            
...
{   
    ...
    for (; dev->finished_copy_counter < dev->copy_counter; )
    {
        req = &dev->copy_task[dev->finished_copy_counter % MAX_COPY_TASK];
