...
#endif
unsigned long rtc_base_date_second;
...
unsigned long venus_rtc_get_time(void)
{
	...
	return (((day*24 + hour)*60 + min)*60 + sec + rtc_base_date_second);
}

int venus_rtc_set_time(unsigned long second)
{
	int day, hour, min, sec, hms;
	unsigned long off_sec = second - rtc_base_date_second;
	
	if(rtc_base_date_second > second) {
		printk("RTC set time error! The time cannot be set to the date before year %d.\n", platform_info.rtc_base_date_year);
		return -EINVAL;
	}
	...
	if(day > 16383) {
		printk("RTC day field overflow. I am so surprised that the Realtek product has been used for more than 40 years. Is it still workable? Hahaha...\n");
		return -EINVAL;
	}
...
unsigned long venus_rtc_alarm_get_time(void)
{
	...
	return (((day*24 + hour)*60 + min)*60 +rtc_base_date_second);
}
...
#endif
	rtc_base_date_second = mktime(platform_info.rtc_base_date_year, 1, 1, 0, 0, 0);
